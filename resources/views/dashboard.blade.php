            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                @role('admin')
                    <div class="bg-blue-100 p-4 rounded-lg">
                        ...
                        <p class="text-2xl font-bold text-blue-600">{{ \App\Models\Account::count() }}</p>
                    </div>
                @endrole
                ...
                @role('staff,admin')
                    <div class="bg-green-100 p-4 rounded-lg">
                        ...
                        <p class="text-2xl font-bold text-green-600">{{ \App\Models\Transaction::whereDate('transaction_date', today())->count() }}</p>
                    </div>

                    <div class="bg-yellow-100 p-4 rounded-lg">
                        <h3 class="text-lg font-semibold text-yellow-800">Total Transaksi</h3>
                        <p class="text-2xl font-bold text-yellow-600">{{ \App\Models\Transaction::count() }}</p>
                    </div>

                    <div class="bg-purple-100 p-4 rounded-lg">
                        <h3 class="text-lg font-semibold text-purple-800">Total Jurnal</h3>
                        <p class="text-2xl font-bold text-purple-600">{{ \App\Models\TransactionDetail::count() }}</p>
                    </div>
                @endrole
            </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach(\App\Models\Transaction::with('details')->latest()->take(5)->get() as $transaction)
                                    <tr>
