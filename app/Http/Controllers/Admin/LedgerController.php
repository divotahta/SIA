...
class LedgerController extends Controller
{
    ...
    public function show(Account $account, Request $request)
    {
        ...
        if ($request->has('export')) {
            $pdf = PDF::loadView('admin.ledger.pdf', compact('account', 'transactions', 'saldoAwal', 'year', 'month'));
            return $pdf->download('buku-besar-' . $account->code . '-' . $year . ($month ? '-' . $month : '') . '.pdf');
        }

        return view('admin.ledger.show', compact('account', 'transactions', 'saldoAwal', 'year', 'month'));
    }
}
